...
def vad_collector(sample_rate, frame_duration_ms,
    ...
    ring_buffer = collections.deque(maxlen=1000)
    ...
    for nf, frame in enumerate(frames):
        ...
        if not triggered:
            ...
            if num_unvoiced > 0.9 * 100:
                #sys.stdout.write('-(%s)' % (frame.timestamp + frame.duration))
                triggered = False
                yield b''.join([f.bytes for f in voiced_frames]), nf
                ring_buffer.clear()
                voiced_frames = []
    #if triggered:
    #    sys.stdout.write('-(%s)' % (frame.timestamp + frame.duration))
...
def main(args):
    if len(args) != 2:
        sys.stderr.write(
            'Usage: example.py <out_dir> <path to wav file>\n')
        sys.exit(1)
